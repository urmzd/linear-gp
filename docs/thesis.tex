...
\begin{description}
    \item{\textbf{default\_fitness}:} The default fitness value assigned to individuals in the population. This value is used to initialize the fitness of all individuals in the population. 
    \item{\textbf{population\_size}:} The number of individuals in each generation of the genetic algorithm. 
    \item{\textbf{gap}:} The percentage of the population that is replaced with offspring in each generation. 
    \item{\textbf{mutation\_percent}:} The percentage of individuals in the population who will be generated from mutation. 
    \item{\textbf{crossover\_percent}:} The percentage of individuals in the population who will be generated from crossover. 
    \item{\textbf{n\_generations}:} The number of generations the genetic algorithm will run for. 
    \item{\textbf{n\_trials}:} The number of times the genetic algorithm will be run with the same settings. 
    \item{\textbf{seed}:} The seed used for random number generation. 
    \item{\textbf{max\_instructions}:} The maximum number of instructions in the genetic program. 
    \item{\textbf{n\_extras}:} The number of extra instructions added to the instruction set. 
    \item{\textbf{external\_factor}:} A factor used to determine the probability of selecting an external instruction during program generation. 
    \item{\textbf{n\_actions}:} The number of possible actions the program can perform. 
    \item{\textbf{n\_inputs}:} The number of inputs the program can take. 
    \item{\textbf{alpha}:} The initial learning rate for the Q-learning algorithm. 
    \item{\textbf{gamma}:} The discount factor for future rewards in the Q-learning algorithm. 
    \item{\textbf{epsilon}:} The probability of selecting a random action in the Q-learning algorithm. 
    \item{\textbf{alpha\_decay}:} The rate at which the learning rate decays over time in the Q-learning algorithm. 
    \item{\textbf{epsilon\_decay}:} The rate at which the exploration rate decays over time in the Q-learning algorithm. 
\end{description}
...
The program generation operation (Algorithm \ref{alg:generate}) creates a new program by generating a random number of instructions and a register set, consisting of action registers (A) and working registers (W). The resulting program is then returned.
...
\begin{algorithm}[hb]
	...
	\begin{algorithmic}[1]
		...
		\Comment we select individuals through tournament selection and apply variation operations
		\EndFor
...
\csvautolongtable[
  table head={
      \caption{Cart Pole RLGP Aggregated Results}
      \label{table:cart-pole-rlgp-results}
