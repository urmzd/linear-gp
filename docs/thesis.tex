...
\begin{algorithm}[hb]
	...
	\begin{algorithmic}[1]
		\State{$score \gets 0$}
		\For{$I \in inputs$}
		\State{$\Call{Execute}{P, I}$}
		\State{$pred \gets \Call{Argmax}{output.action\_registers}$}
		\If{$pred = I$}
		\State{$score \pm 1$}
		\EndIf
		\EndFor
...
\begin{algorithm}[hb]
	...
	\begin{algorithmic}[1]
		...
		\State{$Reward, Done \gets Sim(Action, Environment)$}
		\If{Done}
			\State break
		\EndIf
...
Moreover, during register selection, we apply a greedy selection policy with probability $1-\epsilon$. $\epsilon$ exists to allow us to configure the exploration-exploitation factor, i.e., maintain register selection diversity.
